...
import { LoadFavorites, RemoveFavorite, FavoritesState } from '../store/favorites.state';
...
export class Favorites implements OnInit {
  ...
  loadFavorites() {
    this.loading = true;
    const favoriteIds = this.store.selectSnapshot(FavoritesState.favoriteIds);
    ...
    const requests = favoriteIds.map((id) =>
      this.pollutionService.getById(id).pipe(catchError(() => of(null))),
    );
